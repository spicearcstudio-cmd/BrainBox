...
export default function LevelUpOverlay({ visible, level, title, onDone }: Props) {
  ...
  useEffect(() => {
    ...
    Animated.sequence([
      ...
      Animated.sequence([
        Animated.timing(wobble, { toValue: 1, duration: 100, useNativeDriver: true }),
  ...
  const wobbleRotate = wobble.interpolate({ inputRange: [-1, 0, 1], outputRange: ['-5deg', '0deg', '5deg'] });

  return (
    <Animated.View style={[styles.overlay, { backgroundColor: t.overlay, opacity }]}>
      <Animated.View style={[styles.badge, { backgroundColor: t.gold, transform: [{ scale }, { rotate: wobbleRotate }] }]}>
...
const styles = StyleSheet.create({
  ...
  badge: { alignItems: 'center', padding: 34, borderRadius: 28, minWidth: 200 },
  star: { fontSize: 56, marginBottom: 8 },
